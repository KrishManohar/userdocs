...
#
# The below if section is the restart job for your default $appname installation. This will only manage the default installation.
#
...
if [[ "$(ps -xU $(whoami) | grep -Ecw "$apppath$")" -ne '1' && -d "$HOME/.$appname" && ! -f "$HOME/.userdocs/tmp/$appname.lock" ]]; then
    ...
    screen -dmS "$appname" && screen -S "$appname" -p 0 -X stuff "$screencommand"
    #
