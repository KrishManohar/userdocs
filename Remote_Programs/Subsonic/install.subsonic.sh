...
maxmemory="4096"
#
subsonicfv="https://downloads.sourceforge.net/project/subsonic/subsonic/6.1/subsonic-6.1-standalone.tar.gz"
subsonicfvs="subsonic-6.1-standalone.tar.gz"
# ffmpeg files
...
cronjobadd () {
    ...
    if [[ "$(crontab -l 2> /dev/null | grep -oc '^\* \* \* \* \* bash -l ~/.userdocs/cronjobs/subsonic.cronjob 2>&1$')" == "0" ]]
    then
        ...
        echo '* * * * * bash -l ~/.userdocs/cronjobs/subsonic.cronjob 2>&1' >> "$tmpcron"
        crontab "$tmpcron"
        rm "$tmpcron"
    else
...
cronjobremove () {
    tmpcron="$(mktemp)"
    if [[ "$(crontab -l 2> /dev/null | grep -oc '^\* \* \* \* \* bash -l ~/.userdocs/cronjobs/subsonic.cronjob 2>&1$')" == "1" ]]
    then
        crontab -l 2> /dev/null > "$tmpcron"
        sed -i '/^\* \* \* \* \* bash -l ~\/.userdocs\/cronjobs\/subsonic.cronjob 2>&1$/d' "$tmpcron"
        sed -i '/^$/d' "$tmpcron"
        crontab "$tmpcron"
        rm "$tmpcron"
    else
