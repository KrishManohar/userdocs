...
genericproxypass () {
	...
	# If the proxypas requires a socket.
	socket="1"
	if [[ -d ~/.nginx ]]
	then
...
#
installnode () {
	wget -qO ~/.userdocs/tmp/node.js.tar.gz https://nodejs.org/dist/v6.10.3/node-v6.10.3-linux-x64.tar.xz
	tar xf ~/.userdocs/tmp/node.js.tar.gz --strip-components=1 -C ~/
	cd && rm -rf ~/.userdocs/tmp/node.js.tar.gz
	echo -n "$(~/bin/node -v | sed -rn 's/v(.*)/\1/p')" > ~/.userdocs/versions/$appname.version
}
#
installflood () {
	if [[ ! -d ~/.flood ]]
	then
		...
		wget -qO ~/.flood/config.js "$floodconfig"
		cd ~/.flood
		npm install --production
	fi
	[[ -d ~/.flood ]] && echo "Flood is already installed. Please use the remove or update options"
}
#
generichosturl () {
    echo -e "\033[32m""https://$(hostname -f)/$(whoami)/$appname/""\e[0m"
}
#
genericrestart () {
    #
    ...
    then
        screen -dmS "$appname" npm start --prefix ~/."$appname"
