...
[[ $(dpkg -s libunwind8) ]] && echo 'OK' || echo 'Not OK'
...
mkdir -p ~/lib && cp -rf ~/tmp/usr/lib/x86_64-linux-gnu/* ~/lib
...
[[ $(cat ~/.bashrc | grep -cow '^export LC_ALL=en_US.UTF-8"$') -eq 0 ]] && echo 'export LC_ALL=en_US.UTF-8' >> ~/.bashrc && source ~/.bashrc || :
[[ $(cat ~/.bashrc | grep -cow '^export LANG=en_US.UTF-8"$') -eq 0 ]] && echo 'export LANG=en_US.UTF-8' >> ~/.bashrc && source ~/.bashrc || :
[[ $(cat ~/.bashrc | grep -cow '^export LANGUAGE=en_US.UTF-8"$') -eq 0 ]] && echo 'export LANGUAGE=en_US.UTF-8' >> ~/.bashrc && source ~/.bashrc || :

# Download ombi

wget -qO ~/ombi.linux.tar.gz https://github.com/tidusjar/Ombi/releases/download/v3.0.3111/linux.tar.gz
