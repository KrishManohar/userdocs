...
genericproxypass () {
    ...
    then
        ...
        #
        [[ "$appname" = "sonarr" && -f ~/.config/NzbDrone/config.xml ]] && sed -i 's|PORT|'"$(sed -rn 's|(.*)<Port>(.*)</Port>|\2|p' ~/.config/NzbDrone/config.xml)"'|g' ~/.apache2/conf.d/"$appname".conf
        [[ "$appname" = "jackett" && -f ~/.config/Jackett/ServerConfig.json ]] && sed -i 's|PORT|'"$(sed -rn 's|(.*)"Port": (.*),|\2|p' ~/.config/Jackett/ServerConfig.json)"'|g' ~/.apache2/conf.d/"$appname".conf
        [[ "$appname" = "emby" && -f ~/.emby/ProgramData-Server/config/system.xml ]] && sed -i 's|PORT|'"$(sed -rn 's|(.*)<PublicPort>(.*)</PublicPort>|\2|p' ~/.emby/ProgramData-Server/config/system.xml)"'|g' ~/.apache2/conf.d/"$appname".conf
        #
        [[ "$appname" = "sonarr" && ! -f ~/.config/NzbDrone/config.xml ]] && proxyport="$appport"; sed -i 's|PORT|'"$proxyport"'|g' ~/.apache2/conf.d/"$appname".conf
        [[ "$appname" = "jackett" && ! -f ~/.config/Jackett/ServerConfig.json ]] && proxyport="$jackettappport"; sed -i 's|PORT|'"$proxyport"'|g' ~/.apache2/conf.d/"$appname".conf
        [[ "$appname" = "emby" && ! -f ~/.emby/ProgramData-Server/config/system.xml ]] && proxyport="$embyappporthttp"; sed -i 's|PORT|'"$proxyport"'|g' ~/.apache2/conf.d/"$appname".conf
        #
        ...
        then
            ...
            #
            [[ "$appname" = "sonarr" && -f ~/.config/NzbDrone/config.xml ]] && sed -i 's|PORT|'"$(sed -rn 's|(.*)<Port>(.*)</Port>|\2|p' ~/.config/NzbDrone/config.xml)"'|g' ~/.nginx/conf.d/000-default-server.d/"$appname".conf
            [[ "$appname" = "jackett" && -f ~/.config/Jackett/ServerConfig.json ]] && sed -i 's|PORT|'"$(sed -rn 's|(.*)"Port": (.*),|\2|p' ~/.config/Jackett/ServerConfig.json)"'|g' ~/.nginx/conf.d/000-default-server.d/"$appname".conf
            [[ "$appname" = "emby" && -f ~/.emby/ProgramData-Server/config/system.xml ]] && sed -i 's|PORT|'"$(sed -rn 's|(.*)<PublicPort>(.*)</PublicPort>|\2|p' ~/.emby/ProgramData-Server/config/system.xml)"'|g' ~/.nginx/conf.d/000-default-server.d/"$appname".conf
            #
            [[ "$appname" = "sonarr" && ! -f ~/.config/NzbDrone/config.xml ]] && proxyport="$appport"; sed -i 's|PORT|'"$proxyport"'|g' ~/.nginx/conf.d/000-default-server.d/"$appname".conf
            [[ "$appname" = "jackett" && ! -f ~/.config/Jackett/ServerConfig.json ]] && proxyport="$jackettappport"; sed -i 's|PORT|'"$proxyport"'|g' ~/.nginx/conf.d/000-default-server.d/"$appname".conf
            [[ "$appname" = "emby" && ! -f ~/.emby/ProgramData-Server/config/system.xml ]] && proxyport="$embyappporthttp"; sed -i 's|PORT|'"$proxyport"'|g' ~/.nginx/conf.d/000-default-server.d/"$appname".conf
            #
...
generichosturl () {
    if [[ $(hostname -f | egrep -co ^.*\.feralhosting\.com) -eq "1" ]]
    then
        [[ "$appname" = "sonarr" && -f ~/.config/NzbDrone/config.xml ]] && echo -e "\033[32m""https://$(hostname -f)/$(whoami)/sonarr/settings/general""\e[0m"
        [[ "$appname" = "jackett" && -f ~/.config/Jackett/ServerConfig.json ]] && echo -e "\033[32m""https://$(hostname -f)/$(whoami)/jackett/""\e[0m"
        [[ "$appname" = "emby" && -f ~/.emby/ProgramData-Server/config/system.xml ]] && echo -e "\033[32m""https://$(hostname -f)/$(whoami)/emby/""\e[0m"
    else
        [[ "$appname" = "sonarr" && -f ~/.config/NzbDrone/config.xml ]] && echo -e "\033[32m""http://$(hostname -f):$(sed -rn 's|(.*)<Port>(.*)</Port>|\2|p' ~/.config/NzbDrone/config.xml)/$(whoami)/sonarr/settings/general""\e[0m"
        [[ "$appname" = "jackett" && -f ~/.config/Jackett/ServerConfig.json ]] && echo -e "\033[32m""http://$(hostname -f):$(sed -rn 's|(.*)"Port": (.*),|\2|p' ~/.config/Jackett/ServerConfig.json)/$(whoami)/jackett/admin/dashboard""\e[0m"
        [[ "$appname" = "emby" && -f ~/.emby/ProgramData-Server/config/system.xml ]] && echo -e "\033[32m""http://$(hostname -f):$(sed -rn 's|(.*)<PublicPort>(.*)</PublicPort>|\2|p' ~/.emby/ProgramData-Server/config/system.xml)/""\e[0m"
    fi    
}
#
genericrestart () {
    ...
    then
        [[ "$appname" = "sonarr" ]] && screen -dmS $appname ~/bin/mono --debug ~/.sonarr/NzbDrone.exe
        [[ "$appname" = "jackett" ]] && screen -dmS $appname ~/bin/mono --debug ~/.jackett/JackettConsole.exe
        [[ "$appname" = "emby" ]] && screen -dmS $appname ~/bin/mono --debug ~/.emby/MediaBrowser.Server.Mono.exe
...
do
    ...
    case "$CHOICE" in
        ...
        "2")
            ...
            then
                [[ -f ~/.sonarr/NzbDrone.exe ]] && echo "Updating Sonarr"; echo || echo "Installing Sonarr"
                #
                wget -qO ~/.userdocs/tmp/NzbDrone.tar.gz "$sonarrurl"
                tar xf ~/.userdocs/tmp/NzbDrone.tar.gz -C ~/.userdocs/tmp/
                cp -rf ~/.userdocs/tmp/NzbDrone/. ~/.sonarr
