...
cronjobadd () {
    ...
    if [[ "$(crontab -l 2> /dev/null | grep -oc '^\* \* \* \* \* bash -l ~/.userdocs/cronjobs/'"$appname"'.cronjob$')" == "0" ]]
    then
        ...
        echo '* * * * * bash -l ~/.userdocs/cronjobs/'"$appname"'.cronjob' >> "$tmpcron"
        crontab "$tmpcron"
        rm "$tmpcron"
    else
...
cronjobremove () {
    tmpcron="$(mktemp)"
    if [[ "$(crontab -l 2> /dev/null | grep -oc '^\* \* \* \* \* bash -l ~/.userdocs/cronjobs/'"$appname"'.cronjob$')" == "1" ]]
    then
        crontab -l 2> /dev/null > "$tmpcron"
        sed -i '/^\* \* \* \* \* bash -l ~\/.cronjobs\/'"$appname"'.cronjob$/d' "$tmpcron"
        sed -i '/^$/d' "$tmpcron"
        crontab "$tmpcron"
        rm "$tmpcron"
    else
...
prerequisites () {
    mkdir -p ~/bin
    mkdir -p ~/.userdocs/{versions,cronjobs,credentials,logs,pids,tmp}
...
generichosturl () {
    if [[ $(hostname -f | egrep -co ^.*\.feralhosting\.com) -eq "1" ]]
    then
        [[ "$appname" = "sonarr" && -f ~/.config/NzbDrone/config.xml ]] && echo -e "https://$(hostname -f)/$(whoami)/sonarr/settings/general"
        [[ "$appname" = "jackett" && -f ~/.config/Jackett/ServerConfig.json ]] && echo -e "https://$(hostname -f)/$(whoami)/jackett/"
    ...
    case "$CHOICE" in
        "1")
            ...
            then
                ...
                cd && rm -rf ~/.userdocs/tmp/mono{-"$monovshort",.tar.gz}
                #
        ...
        "2")
            ...
            then
                ...
                genericrestart
                #
                [[ -f ~/.config/NzbDrone/config.xml ]] && generichosturl
                #
                echo
